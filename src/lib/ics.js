...
function parseDate(value, params) {
  ...
  var iso =
    ...
    match[6];
  if (match[7] === "Z") {
    return { date: new Date(iso + "Z"), allDay: false, tz: "UTC" };
  }
  if (tz) {
    return { date: new Date(iso + "Z"), allDay: false, tz: tz, floating: true };
  }
...
function parseIcs(text) {
  ...
  lines.forEach(function (line) {
    ...
    if (key === "X-WR-CALNAME" || key === "CALNAME") {
...
function isDeclined(event, selfEmail) {
  ...
  event.attendees.forEach(function (a) {
    if (a.partstat === "DECLINED" && (!mine || a.email === mine)) {
      declined = true;
    }
  });
  return declined;
